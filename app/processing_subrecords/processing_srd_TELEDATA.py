    ...
    for srd in srds:
        ...
        # Это надо
            ...
            (
                ...
                data_srd["FIX"],
                data_srd["VLD"],
            ) = param_bit(tmp_byte, (1, 1, 1, 1, 1, 1, 1, 1))
            ...
            data_for_db.update_pos_data(
                hex_to_dec(data_srd["NTM"]),
                hex_to_dec(data_srd["LAT"]),
        ...
        # это надо
            ...
            for ind_bit in range(len(bit_list_DIOE)):
                if int(bit_list_DIOE[ind_bit]) == 1:
                    (
                        dict_srd["SRD"],
                        data_srd["ADIO{ind}".format(ind=ind_bit + 1)],
                    ) = param_byte(dict_srd["SRD"], 1, False)
            ...
            for ind_bit in range(len(bit_list_ASFE)):
                if int(bit_list_ASFE[ind_bit]) == 1:
                    (
                        dict_srd["SRD"],
                        data_srd["ANS{ind}".format(ind=ind_bit + 1)],
                    ) = param_byte(dict_srd["SRD"], 3, False)
        ...
        # это не надо. но оно используется?.
            ...
            while len(dict_srd["SRD"]) > 0:
                ...
                data_srd["ADS{i}".format(i=i)], srd_dict_data["RTM"] = param_byte(data_srd["ADS{i}".format(i=i)], 2, True)
                data_srd["ADS{i}".format(i=i)], srd_dict_data["XAAV"] = param_byte(data_srd["ADS{i}".format(i=i)], 2, True)
                data_srd["ADS{i}".format(i=i)], srd_dict_data["YAAV"] = param_byte(data_srd["ADS{i}".format(i=i)], 2, True)
                data_srd["ADS{i}".format(i=i)], srd_dict_data["YAAV"] = param_byte(data_srd["ADS{i}".format(i=i)], 2, True)
