...
logs_path = os.getenv("LOGS_PATH", "egts-device-logs/")
    ...
    # Если файлы есть в директории.
        ...
        if round(int(os.stat(path + last_file).st_size) / 1024 / 1024, 2) >= 48:
            log_file = log_file.format(
                file=current_datetime.strftime("%d-%m-%Y"),
                num=int(last_file.split(".")[1]) + 1,
            )
...
class Logging:
    participants = {1: None, 2: "Server", 3: ""}
        ...
        with open(path, "a", encoding="utf-8") as file:
            file.write(
                "{time} | {fromm} -> ".format(
                    ...
                    fromm=self.participants[fromm],
                )
                + "{to} | {type_msg} | {text}\n".format(to=self.participants[to], type_msg=messages[type_text], text=text)
            )
